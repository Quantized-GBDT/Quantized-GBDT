...
void CUDARegressionL1loss::Init(const Metadata& metadata, data_size_t num_data) {
  ...
  AllocateCUDAMemory<data_size_t>(&cuda_data_indices_buffer_, static_cast<size_t>(num_data), __FILE__, __LINE__);
  AllocateCUDAMemory<label_t>(&cuda_percentile_result_, 1, __FILE__, __LINE__);
  if (cuda_weights_ != nullptr) {
    const int num_blocks = (num_data_ + GET_GRADIENTS_BLOCK_SIZE_REGRESSION - 1) / GET_GRADIENTS_BLOCK_SIZE_REGRESSION + 1;
    AllocateCUDAMemory<double>(&cuda_weights_prefix_sum_, static_cast<size_t>(num_data), __FILE__, __LINE__);
    AllocateCUDAMemory<double>(&cuda_weights_prefix_sum_buffer_, static_cast<size_t>(num_blocks), __FILE__, __LINE__);
    AllocateCUDAMemory<label_t>(&cuda_weight_by_leaf_buffer_, static_cast<size_t>(num_data_), __FILE__, __LINE__);
  }
  AllocateCUDAMemory<double>(&cuda_residual_buffer_, static_cast<size_t>(num_data_), __FILE__, __LINE__);
}
...
void CUDARegressionMAPELOSS::Init(const Metadata& metadata, data_size_t num_data) {
  ...
  AllocateCUDAMemory<label_t>(&cuda_label_weights_, static_cast<size_t>(num_data_), __FILE__, __LINE__);
